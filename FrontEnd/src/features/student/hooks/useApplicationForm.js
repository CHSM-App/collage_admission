/**
 * useApplicationForm — manages the multi-step application form state.
 *
 * Encapsulates:
 * - useReducer with initialState + reducer
 * - init() effect (5 API calls: initApplication, getApplicationForm, getStudentAutofill, getStudentDocuments, getRequiredDocuments)
 * - buildAutofill() merge function
...
function buildAutofill(app, lastApp, profile, user) {
  ...
  return {
    ...
    native_state:        appPrefix('native_state'),
...
export function useApplicationForm() {
    ...
    async function init() {
      ...
      try {
        ...
        if (!appId) {
          ...
          const initRes = await initApplication({
            student_id:          user.id,
        ...
        const sdRes = await getStudentDocuments(user.id)
  ...
  function setDocuments(linked, studentDocs) {
    dispatch({
      type: 'SET_DATA',
      patch: { linked_documents: linked, ...(studentDocs && { student_documents: studentDocs }) },
    })
  }
